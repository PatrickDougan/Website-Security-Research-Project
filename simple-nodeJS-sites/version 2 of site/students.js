module.exports = function(){
    ...
    function getStudent(res, mysql, context, id, complete){
        var sql = "SELECT id, fname, lname, grade, bid FROM students WHERE id = ?";
        var inserts = [id];
        mysql.pool.query(sql, inserts, function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
...
/*      context.jsscripts = ["updatestudent.js"];     */
    ...
    router.post('/', function(req, res){
        var mysql = req.app.get('mysql');
        var sql = "INSERT INTO students (fname, lname, grade, bid) VALUES (?,?,?,?)";
        var inserts = [req.body.fname, req.body.lname, req.body.grade, req.body.my_band];
        sql = mysql.pool.query(sql,inserts,function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
            }else{
                res.redirect('/test_site2/students');
            }
        });
    });
    ...
    router.put('/:id', function(req, res){
        var mysql = req.app.get('mysql');
        var sql = "UPDATE students SET fname=?, lname=?, grade=?, bid=? WHERE id=?";
        var inserts = [req.body.fname, req.body.lname, req.body.grade, req.body.my_band, req.params.id];
        sql = mysql.pool.query(sql,inserts,function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
